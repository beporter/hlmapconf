...
# hlmapconf.pl                                                              #
#                                                                           #
#   - A script to set custom configs for maps in an HL mapcycle.            #
...
#   - Part of SF Publisher v0.90                                            #
#############################################################################
# CVS $Id$ #
#############################################################################


### DEVELOPMENT BOOKMARK CODES ##############################################
# Search for these to locate 'bookmarks'.
...
my $VERSION             = "0.90";
my $RELEASE_DATE        = "November 26, 2002";
...
### GLOBAL VARIABLES ########################################################
# Modify these varaibles to suit your environment. Use unix style forward
# slashes for unix AND Windows path names.

# HLDS base directory name.   IE: "C:/Sierra/Half-Life/"
#my $hldsBaseDir         = "/usr/local/games/hlds_l/";  # Unix
#my $hldsBaseDir         = "C:/Sierra/Half-Life/";      # Windows
my $hldsBaseDir         = "C:/Games/Half-Life/"; #CUSTOM#
...
# Your server's mapcycle file name. Usually 'mapcycle.txt'.
...
if   ($opts{"v"}){ VersionAndUsage();     }
elsif($opts{"i"}){ Mode_CmdInteractive(); }
...
# Command Line Interactice Mode. Reads the mapcycle and allows the user to
# edit cvars for any map listed. Dependent on MapMenu() and ConfigMenu()
# to return user selections.
{
...
#---------------------------------------------------------------------------#
sub Mode_CmdBatch
# Command Line Batch Mode. Takes the list of maps and cvar,value pairs from
# the command line and auto-generates config files for each map containing
# the specified cvars.
{
  my $input = 0;            # Temp selection.
  my $input2 = 0;           # Temp selection.
  ...
  # List of files from cmd line (or mapcycle if no -m switch) to write to.
...
#---------------------------------------------------------------------------#
sub Mode_CmdQuick
# Command Line Quick Mode. Takes the list of maps and cvars from the command
# line and prompts the user for a value for each cvar on each map.
{
  my $input = 0;            # Temp selection.
  my $input2 = 0;           # Temp selection.
  my $temp;                 # Temp var.
...
#---------------------------------------------------------------------------#
sub Mode_TkVisual
# TK Visual Mode. Designed to be a cross platform GUI frontend using the 
# Perl Tk module. Currently unimplemented.
{
  use Tk;
  #INC#
  # Tk GUI commands
...
#---------------------------------------------------------------------------#
sub MapMenu
# Used by the Interactive Mode to process user selections from the mapcycle.
# Returns the index of the map in @mapList selected by the user, or -1 to
# indicate an "exit" status.
{
	my $input = 0;
...
#---------------------------------------------------------------------------#
sub ConfigMenu
# Used by the Interactive Mode to process user selections from a map's list 
# of cvars. Returns the name of the cvar in %cvars, or 0 to indicate an 
# "exit" status.
{
	my $input = 0;
...
Switches:
  -b  ~ [B]atch Mode. Designates a cvar,value[,cvar,value] list. The cvars
        and values specified will be used to auto-generate config files for
        each map listed in the -m switch. If -m is not included, the script
        will try to load the mapcycle.
  -i  ~ [I]nteractive Mode. (Command Line)
  -m  ~ For use with Batch and Quick Modes. Designates a comma seperated list
        of [m]ap names for which to create cfg files.
  ...
  -q  ~ [Q]uick Mode. Designates a cvar[,cvar] list. The user will be prompted
        to enter a value for each cvar specified on each map specified with
        the -m switch. If -m is not included, the script will try to load 
        the mapcycle.
...
Comments:
  ...
  precedence is v,i,q,b,t. To use Interactive Mode, specify -i. To use Tk
...
# Holds a DOS box open if the OS type contains "win".
{
	if($^O =~ m/win/i)
	{
  	print("\nYou are using a Windows command line,\n"
          . "so I'm holding the window open for you.\n\n"
